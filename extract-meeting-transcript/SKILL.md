...
- Check capture completeness: Granola records locally and only while the app is running, so a late start or early stop is normal. Read the first and last turns — does it open at the top of the meeting or mid-sentence, and run through the goodbyes or stop mid-agenda? Record the finding in the Step 5 report; it decides whether Read.ai is needed for gap-fill.
